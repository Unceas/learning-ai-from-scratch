...
## Query Expansion & HyDE Retrieval (Day 148)

Bridges the vocabulary mismatch between informal or high-level user queries and formal academic terminology in research papers (e.g., *"How do transformers remember things from far away in a sentence?"* vs. *"long-range dependency modeling"*).
...
```text
                 ...
                 Query Router
                         │
            ┌────────────┴────────────┐
            │ (QueryType.SEMANTIC)    │ (QueryType.COMPARISON / FACTUAL)
            ▼                         ▼
   HyDE Generator & Expander     Query Decomposition / Direct
   - 1-2 Query Expansions        - 1-4 Subqueries or Original
   - Hypothetical Passage (HyDE)
            │                         │
            └────────────┬────────────┘
                         │
         For each query variant & HyDE:
...
- **Query Expander Service (`backend/services/query_expander.py`)**: Generates 1 to 2 alternative phrasing or terminology expansions using LLM structured prompts with deterministic heuristic fallback.
- **Original Query Preservation**: Retrieval is always executed for `[original_query] + expanded_queries`. Expansion enhances query coverage without ever replacing the user's authentic intent.
- **MAX_EXPANDED_QUERIES Enforced**: Hard-capped at 2 (`MAX_EXPANDED_QUERIES = 2`) to ensure tight latency bounds while eliminating redundant semantic drift.
- **Hypothetical Document Embeddings (HyDE) (`backend/services/hyde.py`)**: Uses the LLM to generate a plausible hypothetical research abstract/passage in the dense embedding space of scientific literature, then embeds it to retrieve real candidate chunks.
- **Hallucination Guard**: The hypothetical passage generated by HyDE is strictly an embedding vector search anchor. It is **never** added to candidate chunks or injected into LLM context, guaranteeing zero hallucinated evidence.
- **Tenant Isolation & Security Boundary**: HyDE dense vector search is strictly scoped to SQLite documents matching `user_id` and `status == 'indexed'`. Unowned or unindexed documents remain inaccessible.
- **Selective Triggering**: HyDE is selectively activated for `QueryType.SEMANTIC` queries where vocabulary mismatch is prevalent, while `FACTUAL` and `COMPARISON` queries remain focused on exact lexical and structural retrieval.
