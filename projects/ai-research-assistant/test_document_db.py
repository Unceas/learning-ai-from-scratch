...
async def main():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        token_a = await get_test_token(client, "doc_db_user_a", "strongpassword123")
        headers_a = {"Authorization": f"Bearer {token_a}"}

        token_b = await get_test_token(client, "doc_db_user_b", "strongpassword123")
        ...
        with patch.object(DocumentService, 'extract_text', return_value=[{"page": 1, "text": "Database-Backed Document Management and RAG metadata content."}]):
            ...
            assert res_upload.json()["status"] in ["indexed", "already_indexed"]
