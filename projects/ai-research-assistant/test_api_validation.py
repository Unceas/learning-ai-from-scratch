...
async def main():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        print("\n--- 4. Testing Empty Document Extraction (AppException 400) ---")
        with patch.object(DocumentService, 'extract_text', return_value=[]):
            ...
            assert res_empty_doc.status_code == 400
            assert res_empty_doc.json() == {
        ...
        with patch.object(DocumentService, 'extract_text', return_value=[{"page": 1, "text": "Valid research document content."}]):
            ...
            assert res_valid.json()["status"] in ["indexed", "already_indexed"]
